...
export default function CharacterQuizApp() {
  ...
  useLayoutEffect(() => { document.title = "Quiz Personnages — Mosaïque"; requestAnimationFrame(() => document.getElementById("main-content")?.focus({ preventScroll: true })); }, []);
  ...
  const validate = () => {
    ...
    if (!complete) { setError(question.responseType === "association" ? "Réponds aux trois associations avant de valider." : "Choisis au moins une réponse avant de valider."); return; }
