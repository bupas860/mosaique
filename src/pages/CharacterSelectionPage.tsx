...
interface Props { characters: readonly PlayableCharacterV2[]; onSelect: (characterId: CharacterIdV2) => void; onBack: () => void; }

const PLAYER_GALLERY_ORDER: readonly CharacterIdV2[] = [
  "P04",
  "P05",
  "P09",
  "P02",
...
export default function CharacterSelectionPage({ characters, onSelect, onBack }: Props) {
  ...
  const displayedCharacters = PLAYER_GALLERY_ORDER.map((id) => charactersById.get(id)).filter(
    (character): character is PlayableCharacterV2 => character !== undefined,
  );
  ...
  return <Screen><main className="w-full max-w-[var(--ds-content-width)] space-y-8 p-4 sm:p-8 lg:p-12">
    ...
    <div aria-label="Personnages à incarner" className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
      {displayedCharacters.map((character) => <article key={character.id} className="character-card interactive-card group relative flex min-w-0 cursor-pointer flex-col overflow-hidden rounded-2xl border focus-within:ring-2 focus-within:ring-blue-600 focus-within:ring-offset-2" style={{ "--character-accent": character.accentColor } as React.CSSProperties}>
