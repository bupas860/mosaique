...
interface Props { initialGameSet: VisibleObstaclesGameSet; selectedCharacterId: CharacterIdV2; selectedModeId: GameModeId; onChooseAnotherCharacter: () => void; onBackHome: () => void; }
...
export default function GamePage({ initialGameSet, selectedCharacterId, selectedModeId, onChooseAnotherCharacter, onBackHome }: Props) {
  if (selectedModeId !== "visible-obstacles") throw new Error(`Aucune banque jouable pour le mode ${selectedModeId}`);
  ...
  if (phase === "end") return <FinalSummaryPage characters={characters} initialCharacters={initialGameCharacters()} playedSituations={gameSet.situations} choiceHistory={choiceHistory} selectedCharacterId={selectedCharacterId} selectedModeId="visible-obstacles" onRestart={restart} onChooseAnotherCharacter={onChooseAnotherCharacter} onBackHome={onBackHome} />;
  const latest = choiceHistory.at(-1);
  const feedback = phase === "feedback" ? getPlayableVisibleObstacleFeedback(situation.id, selectedCharacterId) : undefined;
