...
interface Props { characterId: EditorialCharacterIdV2; characterName: string; image: string | null; accentColor: string; alt?: string; size?: CharacterPortraitSize; className?: string; eager?: boolean; decorative?: boolean; }
...
export default function CharacterPortrait({ characterId, characterName, image, accentColor, alt: explicitAlt, size = "avatar", className = "", eager = false, decorative = false }: Props) {
  const [hasError, setHasError] = useState(false);
  const { width, height } = dimensions[size];
  const alt = explicitAlt ?? getCharacterPortraitAltV2(characterId);
