...
export default function SituationCard({ situation, characterName, disabled = false, showChoices = true, onDecision }: Props) {
  ...
  return <div className="situation-card min-w-0 space-y-6">
    <Illustration type="situation" id={situation.id} source={situation.image} alt={`Illustration de la situation : ${title}`} fallbackLabel="Illustration à venir" className="situation-card__illustration aspect-[5/4] w-full rounded-xl border border-slate-300 shadow-card" />
