...
for (const expected of ['route.kind !== "home" && isEleaContext()', '!eleaPresentation && <nav', '<span className="public-brand">Mosaïque</span>', 'public-footer--elea']) if (!frame.includes(expected)) throw new Error(`Chrome Éléa incomplet : ${expected}`);
...
if (!gallery.includes('!eleaPresentation && <p><a href="#/personnages/quiz"') || !gallery.includes('!eleaPresentation && <a href="#/"')) throw new Error("Allègement de la galerie Personnages incomplet");
if (routes.parseAppRoute("#/personnages/quiz").kind !== "character-quiz") throw new Error("Route directe Quiz Personnages supprimée");

console.log("Contexte Éléa contrôlé : valeur exacte, cinq deep links, accueil normal, chrome réduit et route Quiz Personnages conservée.");
